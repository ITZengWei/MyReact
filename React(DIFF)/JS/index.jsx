...
class ClassCom extends Component {
	...
	render() {
		let { name } = this.props
		let { num, title } = this.state
		console.log(this.props)
		return (
			<div>
				<h1 key={ 1 } title={ title } onClick={ () => { this.setState({ title: title + '-1' }) } }>hello { name } { title }</h1>
				<div className="calcWra">
					<button className="reduce" onClick={ this.handleReduce }>-</button>
					<span>{ num }</span>
					<button className="add" onClick={ this.handleAdd }>+</button>
...
render((
	<div>
		{/*<ClassCom name="bill"><ClassCom name="bill"><strong>hello</strong></ClassCom><strong>hello</strong></ClassCom>*/}
		<ClassCom name="bill"><ClassCom name="tom"></ClassCom></ClassCom>
		{/*{*/}
			{/*[1,2, 3, 4].map(v => <ClassCom name={ v + 'comp' } key={ v }></ClassCom>)*/}
		{/*}*/}
	</div>
), document.querySelector('#root'))
//
